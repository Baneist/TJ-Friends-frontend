...
const startCall = async (socket_id:any) => {
  ...
  const VideoPlayer = () => {
    return (
      <View>
        <View >
          <RTCView style={{ height: height - 91, width: width }} streamURL={remote_stream.toURL()} />
          <Modal
            isVisible={true}
            style={styles.modal}
            hasBackdrop={false}
        ...
        >
            <View style={{
              ...
              <Image source={{ uri: userA.userAvatar }} style={styles.userphoto} />
              <Text>{userA.username}</Text>
              <Button onPress={testA} 
              mode='contained' 
              style={{
